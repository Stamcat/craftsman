...
- Base defaults are emitted in the `cf-base` layer.
- Theme overrides are emitted in the `cf-theme` layer.
- `cf-theme` is declared after `cf-base`, so theme values win when specificity is equal.
...
```css
/* declares the order once: cf-base loses to cf-theme on a tie */
...
2. **Between layers, declaration order wins on a tie**, not specificity. That's why `cf-theme` is declared after `cf-base` — a `.button` rule in `cf-theme` beats a `.button` rule in `cf-base` even though the selectors are equally specific.

### Everything Craftsman ships lives in `cf-base`

Every component stylesheet and the global reset are wrapped in `@layer cf-base { ... }`:
...
- `ThemeProvider` overrides win automatically because they're emitted in `cf-theme`, declared after `cf-base`.
...
<Source language="ts" code={`import type { Theme } from "${PACKAGE_NAME}/styles";
...
<Source language="ts" code={`import type { AppTheme } from "${PACKAGE_NAME}/styles";
...
<Source language="tsx" code={`// app.tsx
import React from "react";
import ReactDOM from "react-dom/client";
import { ThemeProvider } from "${PACKAGE_NAME}/styles";
...
<Source language="ts" code={`import { color } from "${PACKAGE_NAME}/styles";
...
<Source language="ts" code={`import { width } from "${PACKAGE_NAME}/styles";
...
<Source language="ts" code={`import { breakpoint } from "${PACKAGE_NAME}/styles";
