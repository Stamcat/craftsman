...
export const Input: React.FC<InputProps> = (props) => {
    ...
    return (
        <InputWrapper className={clsx("input", className)} label={label} labelPosition={labelPosition} error={error} required={required} style={style} value={value} defaultValue={defaultValue}>
            <span className="input-field" data-has-end-adornment={!isEmpty(endAdornment)} data-has-pre-adornment={!isEmpty(preAdornment)}>
                ...
                <input id={inputId} type={type} className={type} value={value} defaultValue={defaultValue} {...rest} />
